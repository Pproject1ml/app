...
class ChattingController
  ...
  Future<void> updateChatRoomByLocation(
  ...
  Future<void> _disabledAction(ChatRoomInterface chatroom) async {
    try {
      final chatroomId = chatroom.chatroomId;
      final _disabledChatroom = chatroom.copyWith(active: false);
      ...
      await _updateChatRoomState(_disabledChatroom);
  ...
  Future<void> _abledAction(ChatRoomInterface chatroom) async {
    try {
      final chatroomId = chatroom.chatroomId;
      final _abledChatroom = chatroom.copyWith(active: true);
      //1. socket에 구독 요청을 한다.
      chattingRepository.subscribeChatRoom(
          roomId: chatroomId,
          onMessage: onMessageHandler,
      ...
      await _updateChatRoomState(_abledChatroom);

      //3. localDb 에서 active로 바꾼다.
      chattingRepository.updateChatroomLocal(_abledChatroom.toHiveModel());
    } catch (e, s) {
      log(e.toString() + s.toString());
    }
  }
...
// 2. server data를 기준으로 메모리에서와 서버 데이터를 비교한다.
  ...
  void _unsubscribe(
    ...
    chattingRepository.unSubscribeChatRoom(
        roomId: roomId, command: command, headers: _header);
  }
